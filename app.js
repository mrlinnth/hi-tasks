...
class TodoApp {
    ...
    bindEvents() {
        ...
        // Detail inputs
        this.elements.detailTitle.addEventListener('blur', () => this.saveCurrentTask());
        this.elements.detailComplete.addEventListener('change', () => this.saveCurrentTask());
        this.elements.detailDueDate.addEventListener('change', () => this.saveCurrentTask());
        this.elements.detailDescription.addEventListener('blur', () => this.saveCurrentTask());
        this.elements.toggleImportant.addEventListener('click', () => this.toggleImportant());
    ...
    createTaskHTML(task) {
        ...
        return `
            <div class="task-item ${task.completed ? 'completed' : ''} ${task.important ? 'important' : ''}" data-task-id="${task._id}">
    ...
    async toggleImportant() {
        ...
        try {
            ...
            const taskIndex = this.tasks.findIndex(t => t._id === this.currentTask._id);
            if (taskIndex !== -1) {
    ...
    async saveCurrentTask() {
        ...
        this.currentTask.title = this.elements.detailTitle.value.trim() || 'Untitled';
    ...
    handleSyncStatus(status) {
        ...
        if (status.syncing !== undefined) {
            if (status.syncing) {
                ...
                syncStatus.querySelector('.sync-text').textContent = this.isOnline ? 'Online' : 'Offline';
            }
        }
    }
    ...
    async saveApiToken() {
        ...
        try {
            ...
            this.showTokenStatus('Invalid token or network error. Please check and try again.', 'error');
